...
{
    public SpriteRenderer _pickUpSprite; // this get's set in the inspector.
    public GameObject _particlePrefab; // this is spawned when this is destroyed.

    public AudioClip _SFX1; // 4 different pickup audio cues for each type of pickup.
    public AudioClip _SFX2;
    public AudioClip _SFX3;
    public AudioClip _SFX4;

    AudioClip pickUpSFX; // the reference audio clip we will set when instantiated, and call when destroyed.
    ...
    public void Death() // when we destroy this game object, this get's called.
    {
        ...
        GameManager.instance._sfxManager.PlaySFX(pickUpSFX); // call our sfx manager through our game manager.
